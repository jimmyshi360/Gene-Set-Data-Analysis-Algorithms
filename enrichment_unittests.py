...
class TestStattests(unittest.TestCase):
    ...
    def test_gsea(self):
        ...
        cluster=0
        permutations=5
        self.assertAlmostEqual(float(gsea(expr_list,cluster,anno,permutations,1)[0][0].es), 0.629079429538, delta=0.0001)
        self.assertAlmostEqual(float(gsea(expr_list,cluster,anno,permutations,1)[0][1].es), 0.458915389493, delta=0.0001)
    ...
    def test_wilcoxon(self):
        ...
        expr_list = MAT(os.path.join("test_files","CLUSTERS.mat"))
        cluster=0
        self.assertAlmostEqual(float(wilcoxon(expr_list,cluster,anno,1)[0][0].p_value), 1.1206994619e-10, delta=0.0001)
        self.assertAlmostEqual(float(wilcoxon(expr_list,cluster,anno,1)[0][1].p_value), 0.002463584445950147, delta=0.0001)
