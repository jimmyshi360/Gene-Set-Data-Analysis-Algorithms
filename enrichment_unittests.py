...
class TestStattests(unittest.TestCase):
    ...
    def test_gsea(self):
        ...
        cluster=0
        permutations=5
        self.assertAlmostEqual(float(gsea(rankings,anno,cluster,permutations,1)[0][0].es), 0.629079429538, delta=0.0001)
        self.assertAlmostEqual(float(gsea(rankings,anno,cluster,permutations,1)[0][1].es), 0.458915389493, delta=0.0001)
    ...
    def test_generate_inputs(self):
        ...
        self.assertEqual(generate_inputs(anno, cluster, rankings, permutations)[0].go_id,'0')
        self.assertEqual(generate_inputs(anno, cluster, rankings, permutations)[0].gene_list, test_set)
        self.assertEqual(generate_inputs(anno, cluster, rankings, permutations)[0].cluster, 0)
        self.assertEqual(generate_inputs(anno, cluster, rankings, permutations)[0].rankings, rankings)
        self.assertEqual(generate_inputs(anno, cluster, rankings, permutations)[0].permutations, 5)
    ...
    def test_wilcoxon(self):
        ...
        rankings = MAT(os.path.join("test_files","CLUSTERS.mat"))
        cluster=0
        self.assertAlmostEqual(float(wilcoxon(rankings,anno,cluster,1)[0][0].p_value), 1.1206994619e-10, delta=0.0001)
        self.assertAlmostEqual(float(wilcoxon(rankings,anno,cluster,1)[0][1].p_value), 0.002463584445950147, delta=0.0001)
    ...
    def test_page(self):
        ...
        rankings = MAT(os.path.join("test_files", "CLUSTERS.mat"))
        cluster = 0
        self.assertAlmostEqual(float(page(rankings, anno, cluster,1)[0][0].p_value), 6.35801240607e-17, delta=0.0001)
        self.assertAlmostEqual(float(page(rankings, anno, cluster,1)[0][1].p_value), 1.1116067475e-10, delta=0.0001)
